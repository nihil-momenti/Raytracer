...
class Colour
  ...
  def self.toQuantum(floatVal)
    return [0, [Magick::QuantumRange, (Magick::QuantumRange * floatVal).to_i].min].max
  end

  def toPixel
    return Magick::Pixel.new(Colour.toQuantum(@r), Colour.toQuantum(@g), Colour.toQuantum(@b))
  end
...
if __FILE__ == $0
  ...
  p "As pixel, colour A is:", colourA.toPixel()
end
